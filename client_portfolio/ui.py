...
def _coverage_caption(totals: dict) -> str:
    cov = totals.get("data_coverage")
    if cov is None or (isinstance(cov, float) and math.isnan(cov)):
        return ""
    foreign_cov = totals.get("foreign_coverage", 0)
    return (
        f"כיסוי נתונים מינימלי לרבדים המרכזיים: {cov:.0f}% · "
        f"מניות {totals.get('equity_coverage', 0):.0f}% · "
        f"חו\"ל {foreign_cov:.0f}% · "
        f"מט\"ח {totals.get('fx_coverage', 0):.0f}% · "
        f"לא סחיר {totals.get('illiquid_coverage', 0):.0f}%"
    )
...
def render_client_portfolio(df_long: pd.DataFrame, product_type: str) -> None:
    ...
    with st.expander("📊 ניתוח תיק לקוח", expanded=False):
        ...
        k8.metric("כיסוי נתונים",    _fmt(totals.get("data_coverage")))
        st.caption(_coverage_caption(totals))
...
def _render_charts(df: pd.DataFrame, totals: dict) -> None:
    """Render all 9 analysis charts in a clean 2-column grid."""
    from client_portfolio.charts import (
        ...
        chart_concentration, chart_sharpe_comparison, chart_radar,
    )

    # Row 1: manager pie + stocks/bonds
    c1, c2 = st.columns(2)
    with c1:
    ...
    # Row 3: annuity vs capital + radar
    c5, c6 = st.columns(2)
    with c5:
        _safe_plotly(chart_annuity_capital(df), "cp_ac")
    with c6:
        _safe_plotly(chart_radar(df), "cp_radar")

    # Full-width: asset breakdown per product
    st.markdown("---")
    _safe_plotly(chart_asset_breakdown(df), "cp_ab")

    # Full-width: concentration + sharpe (labels are often long)
    _safe_plotly(chart_concentration(df), "cp_conc")
    fig_sh = chart_sharpe_comparison(df)
    if fig_sh.data:
        _safe_plotly(fig_sh, "cp_sharpe")
    else:
        st.info("אין נתוני שארפ למוצרים.")
                ...
                try:
                    ...
                    for sheet in xls.sheet_names:
                        ...
                        for _, row in dc.iterrows():
                            ...
                            inferred_type = "קרנות השתלמות"
                            if "פנס" in sheet_name:
                                inferred_type = "קרנות פנסיה"
                            elif "פוליס" in sheet_name:
                                inferred_type = "פוליסות חיסכון"
                            elif "מנהלים" in sheet_name:
                                inferred_type = "ביטוחי מנהלים"
                            elif "גמל" in sheet_name:
                                inferred_type = "קופות גמל"
        ...
        if raw_import:
            ...
            if new_ct > 0:
                if st.button(f"📥 ייבא {new_ct} מוצרים לניתוח", key="cppage_import", type="primary"):
                    added = import_from_session(st, df_long, "קרנות השתלמות")  # fallback type; per-row type inferred when available
                    if added:
...
def _render_downloads_page(df, totals, client_name, holdings_raw):
    ...
    with dc3:
        ...
        st.download_button("⬇️ CSV", data=csv, file_name="portfolio.csv",
                           mime="text/csv", key="cppage_dl_csv")
